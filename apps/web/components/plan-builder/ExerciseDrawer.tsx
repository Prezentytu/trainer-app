...
const FILTERS: Array<{ id: ExerciseType | "all"; label: string }> = [
...
function DrawerBody({
  ...
  const filtered = useMemo(() => {
    ...
    return typeFilter === "all" ? byName : byName.filter((e) => e.type === typeFilter);
            ...
            {filtered.map((e, idx) => {
              ...
              return (
                <button
                  ...
                  onClick={() => commit(e.id)}
                  className={`flex w-full items-center justify-between gap-3 rounded-[10px] px-3 py-2.5 text-left transition-colors ${
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 font-mono text-xs tabular-nums text-muted">
                      {EXERCISE_TYPE_LABELS[e.type]} · {defaultsLine(e)}
                    </p>
                  </div>
                  {active ? (
